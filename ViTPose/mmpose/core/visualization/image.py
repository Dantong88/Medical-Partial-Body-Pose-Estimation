...
def imshow_keypoints(img,
    ...
    for kpts in pose_result:
        filter_out_list = []
        filter_out_list_skeleton = []
        kpts = np.array(kpts, copy=False)

        # draw each point on image
        if pose_kpt_color is not None:
            assert len(pose_kpt_color) == len(kpts)
            chest_score = kpts[3][-1]
            stomach_score = kpts[4][-1]
            back_score = kpts[-1][-1]
            if (chest_score + stomach_score)/2 < back_score:
                filter_out_list.append(3)
                filter_out_list.append(4)
                filter_out_list_skeleton.append(2)
                filter_out_list_skeleton.append(3)
                filter_out_list_skeleton.append(12)
                filter_out_list_skeleton.append(13)
            else:
                filter_out_list.append(21)
                filter_out_list_skeleton.append(20)
                filter_out_list_skeleton.append(21)
            for kid, kpt in enumerate(kpts):
                if kid in filter_out_list:
                    continue
        ...
        if skeleton is not None and pose_link_color is not None:
            ...
            for sk_id, sk in enumerate(skeleton):
                if sk_id in filter_out_list_skeleton:
                    continue
